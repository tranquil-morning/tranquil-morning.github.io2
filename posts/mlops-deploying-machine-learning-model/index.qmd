...
이를 위해 [`CVLIB`](https://www.cvlib.net/)를 사용할 텐데, 이는 매우 간단하지만 강력한 객체 감지 라이브러리로, [`OpenCV`](https://docs.opencv.org/4.5.1/)와 [`Tensorflow`](https://www.tensorflow.org/)에 의해 구동됩니다.

좀 더 구체적으로, [`numpy 배열`(https://numpy.org/doc/stable/reference/generated/numpy.array.html)로 형식이 지정된 이미지를 받아 반환하는 [`detect_common_objects`](https://docs.cvlib.net/object_detection/) 함수를 사용할 것입니다:

- `bbox`: 감지된 객체의 경계 상자 좌표가 포함된 목록 목록입니다.
